								...
								<div class="panel panel-danger">
									<div class="panel-body row" style="margin-bottom: 14px;">
										...
										<div class="col-sm-7">
											...
											<input type="hidden" class="form-control" id="stok" name="stok" value="">
										...
										<div class="col-sm-2">
											<button class="btn btn-danger btn-block" onclick="push_cart()"><i class="fa fa-shopping-cart"></i></button>
										</div>
									</div>
								</div>
							</div>
							<div class="col-sm-4">
								<div class="panel panel-danger">
									<div class="panel-body row">
										<?php $get_no_inv = $lihat->next_no_order_keluar(DATE("Y-m-d")); ?>
							...
							<div class="col-sm-12">
								<div class="panel panel-danger">
									<div class="panel-heading">
										<h4><i class="fa fa-shopping-cart"></i> Hasil Barang Keluar</h4>
									</div>
									<div class="panel-body">
										...
										<div class="row">
											...
											<div class="col-lg-12">
												<select name="penerima" class="form-control" id="penerima" style="width: 20%;float:right; margin-top:20px; margin-left:20px">
												...
												<label for="penerima" style="float:right;margin-top:25px; margin-left:20px">Penerima</label>
											</div>
										</div>
									
									</div>
									<div class="panel-footer" style="text-align:right">
										<a href="javascript:void(0)" class="btn btn-danger" onclick="save_invoice()"><i class="fa fa-money" aria-hidden="true"></i> Save</a>
									</div>
								</div>
							</div>
									...
									<tbody>
										...
										foreach ($hasil as $isi) {
											...
											<tr>
												...
												<td>
													<button class="btn btn-danger btn-xs" onclick="select_barang_push('<?=$param_array?>')">Pilih</button>
												</td>
											</tr>
											<?php
											$no++;
						...
						<div class="modal-content modal-lg" style=" border-radius:0px;">
							...
							<div class="modal-body">
								...
								<tr>
									<td>Qty</td>
									<td>
										<input type="hidden" class="form-control" id="qty_edit_before">
										<input type="number" class="form-control" id="qty_edit"></br>
								</td>
								</tr>
								<tr>
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-danger" onclick="update_transaksi()">Save</button>
								<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
							</div>
						</div>
					</div>
...
function push_cart(){
	...
	var qty_total = Number(qty);//jika dia mesen dengan barang yang sama harus ditotal qty nya brp. stok nya cukup apa kaga
	
	cart_session.forEach(function(val, key){
		if(val[0]==id_barang){
			qty_total = Number(qty_total)+Number(val[8]);
		}
	});

	if(qty=="" && $("#nama_barang").val() == ""){
		alert("Mohon isi terlebih dahulu form barang dan qty!");
	}else if(qty_total>stok){
...
function modalEdit(key){
	...
	$("#key_edit").val(key);
	$("#id_barang_edit").val(cart_session[key][0]);
	$("#qty_edit_before").val(cart_session[key][7]);
...
function update_transaksi(){
	...
	var qty_before = $("#qty_edit_before").val();
	var stok = 0;
	...
	qty_total = Number(qty_total)-Number(qty_before);
...
function save_invoice(){
	...
	$.ajax({
		type: "POST",
		url: "fungsi/tambah/tambah.php?transaksi_keluar=yes",
		data:{
			no_order : no_order,
